void Shaana object#(0x4ef) ()
{
	...
	// var shaanaBored = gflags[SHAANA_BORED]; conversation timeout not possible in u8
	...
	UI_error_message("IN_PARTY flag is: " + UI_get_item_flag(item, IN_PARTY));
	UI_error_message("=====FLAGS=====");	
	...
	if (event == DOUBLECLICK)
	{
		...
		npc_1st_greet = "@Hello stranger.@";
				
		av_2nd_greet  = "@Hello again Shaana.@";
		npc_2nd_greet = [];
		...
		UI_error_message("~~startConvo npc_2nd_greet: " + npc_2nd_greet);
			...
			var anger_time = UI_get_npc_prop(item, "ANGER");
			var current_day = UI_game_day();
			var current_hour = UI_game_hour();
			var current_time = current_day * 24 + current_hour;
			UI_error_message("anger_time: " + anger_time);
			UI_error_message("current_day: " + current_day);
			UI_error_message("current_hour: " + current_hour);
			UI_error_message("current_time: " + current_time);
			
			if (anger_time > 0 && (current_time - anger_time) >= 6)
			{
				UI_error_message("enough time has passed - reset anger");
				gflags[SHAANA_MAD1] = false;
				gflags[SHAANA_MAD2] = false;
				gflags[SHAANA_MAD3] = false;
				UI_set_npc_prop(item, "ANGER", -anger_time);
				madLevel = 0;
			}
				else
				{
					UI_error_message("not enough time has finished - she mad or never was");
				}
	...
	if (started_talking)
	{
		...
		else if (toranDead && hasHeart)
		{
			...
			item.say("@You speak to no one when the living dead walk the streets. If you are dead, then return to where you once rested. And if you are alive, you had best find shelter from the rampaging ghouls and the quaking ground!@");
			sayGoodbye2(item);
			return;
		}
		else
		{
			UI_error_message("fallback greeting called");
			item.say("Shaana's annoyance with you is made clear by the scowl on her face.");
		}
		...
		if (gflags[SHAANA_NAME] && gflags[SHAANA_AXE] && (!is_first) )
		{
			options = options;
		}
			else if (gflags[SHAANA_NAME] && gflags[SHAANA_AXE])
			{
		...
		if (!UI_get_item_flag(item, IN_PARTY))
		{
	    ...
	    if (madLevel >= 3)
	    {
			...
			say("@Greetings again, stranger. I suppose you wish more of my time?@");


			var choice = chooseFromMenu(["No, sorry. Goodbye.", "Only a moment."]);
			if (choice == "No, sorry. Goodbye.")
			{
				sayGoodbye2(item);
				break;
			}
			else
			{
				...
				var current_time = current_day * 24 + current_hour;
				var anger_time = UI_get_npc_prop(item, "ANGER");
				UI_set_npc_prop(item, "ANGER", current_time - anger_time);
			}
			...
			case "what do you do" (remove):
				UI_error_message("click registered on: *what do you*");
				remove("who was the man I saw you kill");
				...
				var choice = chooseFromMenu(["I am not", "I am simply curious"]);
				var what_do_choice_start;
				
				if (choice == "I am not")
					what_do_choice_start = "@Then by all means stop wasting my time. ";
					else if (choice == "I am simply curious")
						what_do_choice_start = "@I am not here to satisfy an idle curiosity, stranger. ";
			
				say(what_do_choice_start + "One would think you were employed by the scholar, the way you waste time with useless banter.@");
				add(["who is the scholar", "i am not wasting time"]);

			case "who was the man I saw you kill" (remove):
				remove("what do you do");
			...
			case "who is this other" (remove):
				...
				var choice = chooseFromMenu(["Aye I swear", "I swear nothing"]);
				var swearSelection;
				
				if (choice == "Aye I swear")
					{
					UI_error_message("click registered on: *aye, i swear*");
					swearSelection = "@Very well. His name is Mythran. You may find him outside the city walls. His wisdom is great, but I warn you not to visit him for idle prattle such as this. I swear myself that you do not wish to see me angered.@";
					}
						else if (choice == "I swear nothing")
						{
						UI_error_message("click registered on: *i*");
						swearSelection = "@Then I tell nothing.@";
						add("i apologize. i swear");
						}
				UI_error_message("npc should say: " +swearSelection);	
				say(swearSelection);
				UI_error_message("npc said: " +swearSelection);	
...
//				add("yet you study other pursuits");
			...
			case "what do you study":
				UI_error_message("pried value before if: " + pried);
				if (pried)
				{
				say("@My dear stranger, if I have not yet made it perfectly clear that my business is none of your concern, let me do so now. My business, -is- none of your concern.@");
				sayGoodbyeAbrupt(item, "@Farewell.@");
				break;
				}
					else
					{
			...
			case "yet you study other pursuits" (remove):
				pried = true;
				UI_error_message("pried value after setting pried to true: " + pried);
				say("@I have warned you not to pry into my affairs. Do not fool yourself into thinking that I am incapable of performing the duties of my position -- even when the victims are not bound. I have -no- qualms about slaying someone who deserves to die. Though I do not necessarily agree with the Lady's methods.@");
				add("with what do you disagree");
			...
			case "join" (remove):
				UI_error_message("Join case: IN_PARTY flag is: " + UI_get_item_flag(item, IN_PARTY));
				if (!UI_get_item_flag(item, IN_PARTY))
				{
					if (UI_get_array_size(party) < 8)
					{
						UI_add_to_party(item);
						UI_error_message("Added to party: IN_PARTY flag now: " + UI_get_item_flag(item, IN_PARTY));
						say("@Very well.@");
						sayGoodbyeAbrupt(item, "Let us go.");
					}
					else
						say("@Your party is full.@");
				}
				else
					say("@I am already with you.@");

			case "leave" (remove):
				UI_error_message("Leave case: IN_PARTY flag is: " + UI_get_item_flag(item, IN_PARTY));
				if (UI_get_item_flag(item, IN_PARTY))
				{
					if (askYesNo())
					{
						script item
							call leaveParty, SCHEDULE_MOVE;
						UI_error_message("Leaving party: called leaveParty script");
						sayGoodbyeAbrupt(item, "Farewell.");
					}
					else
					{
